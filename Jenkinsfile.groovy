    ...
    stages {
        stage('publish') {
            ...
            steps {
                cleanWs()
                checkout scm
                sh "./gradlew publish -x test -DmavenPublishUrl=${MAVEN_PUBLISH_URL} -DmavenUsername=${MAVEN_CREDENTIALS_USR} -DmavenPassword=${MAVEN_CREDENTIALS_PSW} ${GRADLE_EXTRA_OPTION}"
            }
        }
    }
}
